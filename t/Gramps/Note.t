...
$gramps->_import_notes();

my $expected = count_xml_elements($gramps_file, 'note');

is(scalar keys %{ $gramps->notes },
  $expected, "Imported $expected notes");
ok(scalar keys %{ $gramps->notes } > 0, "notes were imported");
