...
$gramps->_import_sources();

my $expected = count_xml_elements($gramps_file, 'source');

is(scalar keys %{ $gramps->sources },
  $expected, "Imported $expected sources");
ok(scalar keys %{ $gramps->sources } > 0, "Sources were imported");
