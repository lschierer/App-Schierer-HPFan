...
$gramps->_import_citations();

my $expected = count_xml_elements($gramps_file, 'citation');

is(scalar keys %{ $gramps->citations },
  $expected, "Imported $expected citations");
ok(scalar keys %{ $gramps->citations } > 0, "citations were imported");
