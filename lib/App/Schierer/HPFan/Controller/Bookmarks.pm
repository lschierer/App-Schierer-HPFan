...
package App::Schierer::HPFan::Controller::Bookmarks {
  ...
  sub bookmark_index ($c) {
    ...
    $logger->debug("Building index for: '$current_path' with tree: " . Data::Printer::np($bookmarks_tree));
    ...
    for my $route (@allRoutes){
      ...
      $logger->debug(sprintf('entry data for route "%s" is : %s', $route, Data::Printer::np($entry_data) ));
      push @child_entries, {
  ...
  sub bookmark_page ($c) {
    ...
    unless($entry and ref $entry eq 'HASH') {
      $logger->error(__PACKAGE__ . "Cannot get entry for " . $c->req->url->to_abs);
      $c->reply->not_found;
    }
  ...
  sub process_bookmark_file ($self, $app, $file, $baseRoute, $BookmarksTree) {
    ...
    if ($@) {
      ...
      if($content->{name}){
        $title = exists $content->{title} ? $content->{title} : $content->{name};
        ...
        if($file->basename eq 'index.yaml') {
          ...
          $route = sprintf('%s/%s', $dirPath, $content->{name}) unless $content->{name} eq 'Bookmarks';
        }
        ...
        $logger->warn("name is a required key in a yaml bookmarks file. '$file' will be ignored until it is present.");
      }
