...
has 'gramps_export' =>
  sub { return Mojo::Home->new->detect->child('share/data/gramps'); };
has 'gramps_db' =>
  sub { return Mojo::Home->new->detect->child('share/grampsdb/sqlite.db'); };
...
sub initialize ($self) {
  $init_promise //= Mojo::Promise->timer(
    0.1 => sub {
      $self->log_info("⚙️  Running gramps import...");
      $self->gramps->execute_import();
    }
  )->then(sub {
    Mojo::Promise->timer(0.1);
  })->then(sub {
    $self->log_info("⚙️  Building gramps indexes...");
    $self->gramps->build_indexes();
  })->then(sub {
    $self->log_info("✅ gramps import completed.");
    $ready = 1;
  });
