...
async sub render_yaml_tables ($self, $html, $entry) {
  ...
  for my $node ($dom->find('yamltable[source]')->each) {
    ...
    my $data;
    eval {
      $data = LoadFile($yaml_path->stringify);
    };
    if ($@) {
...
sub _build_table_from_yaml ($self, $data) {
  ...
  if (ref($data) eq 'HASH' && exists $data->{columnorder} && exists $data->{rows}) {
    ...
    return '<div class="error">YAML must be an array of objects or a hash with "rows" key</div>';
  }
  ...
  if ($column_order && ref($column_order) eq 'HASH') {
    ...
    @columns = sort { $column_order->{$a} <=> $column_order->{$b} } keys %$column_order;
  } else {
  ...
  # Build table header
  my $thead_cells = join '', map {
    qq{<th class="spectrum-Table-headCell">$_</th>}
  } @columns;
  my $thead = qq{<thead class="spectrum-Table-head"><tr>$thead_cells</tr></thead>};
  ...
  for my $row (@$rows) {
    ...
    push @row_html, qq{<tr class="spectrum-Table-row spectrum-Table-cell--divider">} . join('', @cells) . qq{</tr>};
  }
  
  my $tbody = qq{<tbody class="spectrum-Table-body">} . join('', @row_html) . qq{</tbody>};
