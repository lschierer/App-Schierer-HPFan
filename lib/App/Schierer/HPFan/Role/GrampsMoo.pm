package App::Schierer::HPFan::Role::GrampsMoo;
use Moo::Role;
use v5.42.0;
use utf8::all;
...
has 'gramps_db' => (
  is => 'lazy',
  default => sub { path($FindBin::Bin)->parent->child('share/grampsdb/sqlite.db') }
);
...
sub _format_event_date ($self, $event) {
    ...
    # Handle date ranges
    if ($date->is_range) {
        my $start = $date->start ? $self->_format_single_date($date->start) : 'Unknown';
