...
sub generate_svg {
    ...
    $self->_add_person_and_ancestors($graph, $self->root_person, \%visited, \%family_nodes, 0);
...
sub _add_person_and_ancestors {
    ...
    foreach my $family (@$parent_families) {
        ...
        if ($father) {
            $self->_add_person_and_ancestors($graph, $father, $visited, $family_nodes, $generation + 1);
            $graph->add_edge(
                from => $family_id,
                to => $father->handle,
                class => 'family-node junction',
        ...
        if ($mother) {
            $self->_add_person_and_ancestors($graph, $mother, $visited, $family_nodes, $generation + 1);
            $graph->add_edge(
                from => $family_id,
                to => $mother->handle,
                class => 'family-node junction',
...
sub _clean_svg {
    ...
    $svg =~ s/<svg /<svg class="family-tree" preserveAspectRatio="xMidYMid meet" width="100%" height="100%" /;
