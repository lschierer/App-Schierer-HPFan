...
sub _build_class_lists {
    ...
    $logger->debug(sprintf('buildClassList found %s events to filter.', scalar @events));
    ...
    $logger->debug(sprintf('buildClassList found %s sorting events.', scalar @SortingEvents));
    ...
    foreach my $event (@SortingEvents) {
        ...
        foreach my $person (@{ $self->gramps->people_by_event->{ $event->handle } // [] }) {
            push @matches, $person;
        }
        ...
        $logger->debug(sprintf(
            ...
            scalar @matches,
            $eventDate,
            join(', ', map { $_->display_name } @matches)
        ));
