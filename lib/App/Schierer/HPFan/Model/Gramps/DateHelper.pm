...
class App::Schierer::HPFan::Model::Gramps::DateHelper :
  ...
  method _from_free_text ($txt, $meta) {
    ...
    if ($txt =~ /BET(?:WEEN)?\s+(\d{4}(?:-\d{1,2}(?:-\d{1,2})?)?)\s+AND\s+(\d{4}(?:-\d{1,2}(?:-\d{1,2})?)?)/i) {
      my ($a, $b) = ($1, $2);
      my $sa = _parse_iso_like($a);
      my $sb = _parse_iso_like($b);
      return App::Schierer::HPFan::Model::Gramps::GrampsDate->new(
        start    => App::Schierer::HPFan::Model::Gramps::GrampsDate->new(%$sa, type=>'single'),
        end      => App::Schierer::HPFan::Model::Gramps::GrampsDate->new(%$sb, type=>'single'),
        modifier => 4,  # between
        %$meta,
        type => 'span',
      );
    ...
    # FROM yyyy[-..] [TO yyyy[-..]]
    if ($txt =~ /^FROM\s+(\d{4}(?:-\d{1,2}(?:-\d{1,2})?)?)(?:\s+TO\s+(\d{4}(?:-\d{1,2}(?:-\d{1,2})?)?))?/i) {
      my $sa = _parse_iso_like($1);
      my $ea = $2 ? _parse_iso_like($2) : undef;
      return App::Schierer::HPFan::Model::Gramps::GrampsDate->new(
        start    => App::Schierer::HPFan::Model::Gramps::GrampsDate->new(%$sa, type=>'single'),
        ( $ea ? (end => App::Schierer::HPFan::Model::Gramps::GrampsDate->new(%$ea, type=>'single')) : () ),
        modifier => 5,  # from
        %$meta,
        type => ($ea ? 'range' : 'range'),  # treat open-ended as range with only start
      );
    }
  ...
  method parse ($h) {
    return App::Schierer::HPFan::Model::Gramps::GrampsDate->new(type => 'single')
