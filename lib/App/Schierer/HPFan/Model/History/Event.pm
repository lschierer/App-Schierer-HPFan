...
class App::Schierer::HPFan::Model::History::Event
  ...
  field $raw_date   : param : reader //= undef;   # GrampsDate or Date::Manip::Date (the latter when this was created by ::Model::History::Yaml)
  ...
  method to_string {
    ...
    push @parts, $html_desc  unless not defined $html_desc;
    push @parts, $sources->@*;
  ...
  method _kind_rank ($self) {
    my $k = $self->date_kind // '';
    return exists $self->{kind_sort_order}{$k} ? $self->{kind_sort_order}{$k} : 999;
  }

  method _op_cmp ($a, $b, $swap) {
