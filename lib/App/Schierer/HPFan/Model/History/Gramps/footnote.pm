...
class App::Schierer::HPFan::Model::History::Gramps::footnote
  ...
  method citations_for_event {
    $self->logger->debug(sprintf(
      'there are %s citations for event %s',
      scalar @{ $event->citation_list },
      $event->gramps_id
    ));
    ...
    foreach my $cr ($event->citation_list->@*) {
      ...
      foreach my $rh ($source->reporef_list->@*) {
        ...
        if ("$rht" eq 'Electronic') {
          $isWebCitation = 1;
          push @$repos, $gramps->repositories->{ $rh->ref }
            if defined($gramps->repositories->{ $rh->ref });
        }
      }

      unless ($source) {
        $self->logger->warn(sprintf(
          'citation %s points at non-existant source %s.',
          $cite->gramps_id, $sh
        ));
        next;
      }
      ...
      my $title =
        (defined($source->title) && length($source->title)) ? $source->title
        : scalar(@$repos) ? first { defined($_->name) && length($_->name) }
        @$repos
        : 'Unknown';
      ...
      my $page =
        defined($cite->page) && length($cite->page)
        ? sprintf(
        '<dd class="%s">Page: %s</dd>',
      ...
      my $date =
        defined($cite->date) && $cite->date->year != 9999
        ? sprintf(
        '<dd class="%s">%s</dd>',
